...
\begin{itemize}
    \item \textbf{I/O \& Bus:} Gestione I2C per sensori ambientali (\texttt{sensors\_i2c.h}) e modulo di interfaccia per il display OLED (\texttt{display\_ssd1306.h}), sviluppato come wrapper attorno alla libreria commerciale Adafruit SSD1306.
...
Costituisce l'interfaccia di alto livello (\texttt{ui/}). Gestisce il rendering grafico e la navigazione tra le schermate tramite un approccio modulare basato su \textbf{Views} (\texttt{view\_definitions.h}). Questo strato gestisce la logica di visualizzazione interagendo con i gestori del livello Core tramite l'\texttt{UIManager}, mentre delega il rendering grafico alle API primitive fornite dalla libreria esterna Adafruit GFX, operando direttamente tramite un puntatore all'oggetto \texttt{canvas} condiviso.
% --- SOTTOSEZIONE 2 ---
% --- SOTTOSEZIONE 2 ---
...
La comunicazione tra i livelli, in particolare tra il layer di Storage e l'Application, è mediata da strutture dati thread-safe e code FIFO, garantendo che le scritture su Flash non blocchino il thread principale di visualizzazione.
