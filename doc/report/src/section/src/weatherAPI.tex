...
Il flusso è unidirezionale nell'origine: il PC chiede, la board risponde o
riceve. Non c'è polling autonomo dalla board verso il PC.
...
Il protocollo creato é in puro ASCII, ogni messaggio è terminato da \texttt{'\textbackslash n'}.
...
\texttt{tools/api-weather/pcb\_bridge.py} è il server seriale lato PC che
orchestra l'intera sequenza in un singolo script. Le uniche dipendenze esterne sono \texttt{pyserial} e \texttt{requests} per le API HTTP.
...
\textbf{NOTA 1}: \texttt{time.sleep(5)} dopo l'apertura della porta seriale serve poiché
quando il PC apre la connessione CDC, il Pico riceve un segnale DTR che
arduino-pico interpreta come richiesta di reset. Il firmware riparte da capo,
esegue l'intera sequenza di boot (LittleFS, display, animazione) e solo dopo
...
\texttt{reset\_input\_buffer()} svuota
eventuali residui di output del boot prima di inviare il comando.


\textbf{NOTA 2}: Il loop \texttt{for \_ in range(50)} legge fino a 50 righe cercando quella
che inizia con \texttt{GPS\_DATA:}. Le righe precedenti possono contenere
output di debug del firmware (prefisso \texttt{[STORAGE]}, \texttt{CORE 1:},
ecc.) che vengono silenziosamente scartate. 
