...
L'obiettivo primario è la realizzazione di una piattaforma embedded basata sul SoC Raspberry Pi RP2040 \cite{pico_ds}, ottimizzata per il \textit{data-logging} georeferenziato ad alta frequenza. La documentazione di riferimento, le API sviluppate e i dettagli tecnici sull'architettura firmware sono resi disponibili pubblicamente \cite{ppse_api, ppse_homepage, ppse_github}, unitamente ai report accademici e tecnici di validazione \cite{ppse_academic_report, ppse_technical_ref}. 


Le funzionalità principali del sistema comprendono acquisizione
dati GPS (modulo SAM-M8Q) e sincronizzazione dati meteo da host PC via seriale, lettura di sensori ambientali e inerziali via
I2C, interazione tramite display OLED SSD1306, persistenza
su flash con LittleFS, feedback visivo su strip WS2812B e acustico tramite buzzer PWM.
