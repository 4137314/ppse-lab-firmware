...
L'obiettivo primario è la realizzazione di una piattaforma \textit{embedded} basata su \textit{Raspberry Pi Pico} \cite{pico_ds}, ottimizzata per il \textit{data-logging} georeferenziato ad alta frequenza. La documentazione di riferimento, le API sviluppate e i dettagli tecnici sull'architettura firmware sono resi disponibili pubblicamente \cite{ppse_api, ppse_homepage, ppse_github}, unitamente ai report accademici e tecnici di validazione \cite{ppse_academic_report, ppse_technical_ref}. 

Particolare rilievo viene dato alla capacità del sistema di mantenere la persistenza dei log GNSS e dei parametri ambientali anche in scenari di errore critico, convalidando le scelte progettuali attraverso rigorose metodologie di assemblaggio superficiale (SMT).
