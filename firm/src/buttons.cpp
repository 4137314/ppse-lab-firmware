...
void buttonsInit() {
    ...
    gpio_set_irq_enabled_with_callback(SW_UP, GPIO_IRQ_EDGE_FALL | GPIO_IRQ_EDGE_RISE, true,
                                       &gpio_callback);
...
void buttonsUpdate() {
    ...
    if (ev == KEY_LEFT || ev == KEY_RIGHT) {
        if (checkLeftRightCombo(ev))
            return;  // se combo: toggle e fine
    }
...
bool checkLeftRightCombo(KeyEvent ev) {
    ...
    // pulizia (opzionale)
    if (lastL && (now - lastL > COMBO_MS))
        lastL = 0;
    if (lastR && (now - lastR > COMBO_MS))
        lastR = 0;
