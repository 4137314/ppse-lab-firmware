/**
 * @file buttons.cpp
 * @brief Gestione dell'input utente, sitema di debounce e macchina a stati del
 * menu.
 * * Questo modulo implementa la logica di navigazione tramite pulsanti GPIO,
 * utilizzando interrupt (IRQ) e allarmi hardware per il debounce. Gestisce
 * inoltre la logica di timeout del display e le combo di tasti.
 */
...
bool ambientDisplay =
    true; /**< Stato del display: true se in modalità risparmio/home. */
/**@}*/
...
uint32_t tLeft = 0;
uint32_t tRight = 0;
const uint32_t COMBO_MS =
    300; /**< Finestra temporale (ms) per rilevare una pressione simultanea. */
uint32_t lastActivity = 0; /**< Timestamp dell'ultima interazione utente. */
const uint32_t DISPLAY_TIMEOUT_MS =
    5000000; /**< Timeout (ms) per il ritorno automatico alla Home. */
const uint32_t DEBOUNCE_DELAY =
    40; /**< Delay (ms) per il filtraggio del rimbalzo meccanico. */
/**@}*/
 ...
 * Al rilevamento della pressione, disabilita l'interrupt e avvia un allarme di
 * debounce.
...
void gpio_callback(uint gpio, uint32_t events) {
  ...
  switch (gpio) {
    ...
    case SW_RIGHT: {
      ...
      // Add an alarm callback to be called after a delay specified in
      // milliseconds.
 ...
 * * Verifica se dopo @ref DEBOUNCE_DELAY il pin è ancora basso. In caso
 * positivo, valida la pressione impostando il relativo flag @c _press.
...
void buttonsInit() {
  ...
  gpio_set_irq_enabled_with_callback(
      SW_UP, GPIO_IRQ_EDGE_FALL | GPIO_IRQ_EDGE_RISE, true, &gpio_callback);
  for (uint8_t i = 1; i < (sizeof(SWpin) / sizeof(SWpin[0])); ++i) {
    gpio_set_irq_enabled(SWpin[i], GPIO_IRQ_EDGE_FALL | GPIO_IRQ_EDGE_RISE,
                         true);
  }
...
/**
 * @brief Funzione di update della logica dei menu. Da chiamare nel loop
 * principale.
...
void buttonsUpdate() {
  ...
  // se ritorna false => nessuna combo: puoi eseguire l’azione singola (LEFT o
  // RIGHT)
