 ...
 * * Questo modulo gestisce la ricezione e l'organizzazione dei dati meteo correnti
 * e delle previsioni orarie (forecast) ricevute tramite stringhe formattate CSV.
 */
 ...
 * delle previsioni con NAN (Not A Number) per le temperature e 255 per i codici meteo.
 */
void Weather_Init() {
    wx = {};
    strcpy(wx.city, "NA");
 ...
 * @param pos Posizione corrente nell'indice della stringa (aggiornata dopo la chiamata).
 ...
 * - **WXC**: (Weather Current) Dati meteo attuali (Città, Temp, Vento, Umidità, Codice).
 * - **WXD**: (Weather Daily) Dati forecast per un giorno specifico (Indice, 24 Temp, 24 Codici).
...
void Weather_HandleLine(const String &line) {
    ...
    if (line.startsWith("WXD,")) {
        ...
        // Aggiorna maschera bitwise per verificare se tutti i 7 giorni sono stati ricevuti
        wx.recvMask |= (1u << d);
