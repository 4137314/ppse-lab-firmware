 ...
 * * Questo modulo utilizza la libreria FastLED per gestire una striscia di LED RGB,
 * fornendo segnali visivi per l'inizializzazione del sistema, la navigazione nei menu
 * e gli avvisi di inattività.
 */
...
/** * @brief Array globale che rappresenta lo stato dei colori della striscia LED. 
 */
CRGB leds[NUM_LEDS];

/**
 * @brief Inizializza l'hardware dei LED.
 * * Configura il controller FastLED utilizzando il tipo di LED, il pin e l'ordine 
 * dei colori definiti in @ref leds.h.
 */
void ledsInit() {
 ...
 * * Accende un LED Blu nella posizione corrente e un LED Rosso nella posizione successiva,
 * quindi esegue un rapido lampeggio per confermare il movimento dell'indice.
 * Gestisce automaticamente il rollover della striscia (se l'indice raggiunge @ref NUM_LEDS).
 */
...
/**
 * @brief Esegue un flash giallo per segnalare l'imminente timeout dello schermo.
