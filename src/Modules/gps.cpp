...
bool gpsInit() {
  ...
  int i;
  for (i = 0; i < 1000 && (Serial2.available() == 0); ++i)
    delay(1);
...
bool gpsAcquire(enum minmea_sentence_id sentence_type,
  ...
  for (int i = 0; i < GPS_ACQUIRE_MAX_TRIES; ++i) {
    for (int c = 0; Serial2.available() == 0; ++c)
      delay(1);
...
void print_NMEA_gst(void *frame) {
  ...
  Serial.printf("$xxGST fixed point latitude,longitude and altitude error "
  ...
  Serial.printf("$xxGST floating point degree latitude, longitude and altitude "
...
switch ( minmea_sentence_id(nmea_message->c_str(), true)) {
        case MINMEA_SENTENCE_RMC: {
          ...
          if (minmea_parse_rmc(frame, nmea_message->c_str())) {
            #if DEBUG == 1
            Serial.printf("$xxRMC: raw coordinates and speed: (%d/%d,%d/%d)
%d/%d\n", frame->latitude.value, frame->latitude.scale, frame->longitude.value,
frame->longitude.scale, frame->speed.value, frame->speed.scale);
                        Serial.printf("$xxRMC fixed-point coordinates and speed
scaled to three decimal places: (%d,%d) %d\n", minmea_rescale(frame->latitude,
1000), minmea_rescale(frame->longitude, 1000), minmea_rescale(frame->speed,
1000)); Serial.printf("$xxRMC floating point degree coordinates and speed:
...
&global_parsed_nmea.parsed_gga; if (minmea_parse_gga(frame,
nmea_message->c_str())) { #if DEBUG == 1 Serial.printf("$xxGGA: fix quality:
%d\n", frame->fix_quality); #endif
                      }
                      else {
                        #if DEBUG == 1
...
&global_parsed_nmea.parsed_gst; if (minmea_parse_gst(frame,
nmea_message->c_str())) { #if DEBUG == 1 Serial.printf("$xxGST: raw
latitude,longitude and altitude error deviation: (%d/%d,%d/%d,%d/%d)\n",
                        frame->latitude_error_deviation.value,
frame->latitude_error_deviation.scale, frame->longitude_error_deviation.value,
frame->longitude_error_deviation.scale, frame->altitude_error_deviation.value,
frame->altitude_error_deviation.scale); Serial.printf("$xxGST fixed point
latitude,longitude and altitude error deviation" " scaled to one decimal place:
(%d,%d,%d)\n", minmea_rescale(frame->latitude_error_deviation, 10),
                        minmea_rescale(frame->longitude_error_deviation, 10),
                        minmea_rescale(frame->altitude_error_deviation, 10));
                        Serial.printf("$xxGST floating point degree latitude,
longitude and altitude error deviation: (%f,%f,%f)",
...
&global_parsed_nmea.parsed_gsv; if (minmea_parse_gsv(frame,
nmea_message->c_str())) { #if DEBUG == 1 Serial.printf("$xxGSV: message %d of
%d\n", frame->msg_nr, frame->total_msgs); Serial.printf("$xxGSV: satellites in
view: %d\n", frame->total_sats); for (int i = 0; i < 4; i++)
                        Serial.printf("$xxGSV: sat nr %d, elevation: %d,
...
&global_parsed_nmea.parsed_vtg; if (minmea_parse_vtg(frame,
nmea_message->c_str())) { #if DEBUG == 1 Serial.printf("$xxVTG: true track
degrees = %f\n", minmea_tofloat(frame->true_track_degrees)); Serial.printf("
magnetic track degrees = %f\n", minmea_tofloat(frame->magnetic_track_degrees));
      ...
      /*
      bool gpsAcquire(enum minmea_sentence_id sentence_type, struct parsed_nmea
*nmea_ptr) { String nmea_message;
