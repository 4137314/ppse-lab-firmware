 ...
 * * Questo header contiene le definizioni dei pin per i pulsanti di navigazione,
 * i flag di stato per la macchina a stati del menu e i prototipi per la gestione
 * degli interrupt e del debounce su RP2040.
 */
 ...
 * @{ */
extern int menuIndex;           /**< Indice corrente della voce selezionata nel menu principale. */
extern bool menuOpen;           /**< Stato di visibilità del menu principale. */
extern bool inSubmenu;          /**< Indica se l'utente si trova all'interno di una schermata sottomenu. */
 ...
 * @{ */
extern uint32_t tLeft;             /**< Timestamp dell'ultima pressione sinistra per rilevamento combo. */
extern uint32_t tRight;            /**< Timestamp dell'ultima pressione destra per rilevamento combo. */
extern const uint32_t COMBO_MS;    /**< Finestra temporale massima per validare una pressione simultanea. */
extern uint32_t lastActivity;      /**< Timestamp dell'ultima attività registrata per il timeout. */
...
/**
 * @brief Esegue il polling della logica dei menu e gestisce gli eventi dei tasti.
...
/**
 * @brief Funzione richiamata dall'allarme hardware per confermare la pressione (Debounce).
 * @param id ID dell'allarme hardware Pico.
 * @param user_data Dati utente (generalmente il numero del pin).
 * @return long long Tempo per la prossima esecuzione (0 per esecuzione singola).
 */
