/**
 * @file buzzer.cpp
 * @brief Driver per la gestione del feedback acustico tramite buzzer piezoelettrico.
 * * Fornisce funzioni per l'inizializzazione del pin e la generazione di segnali
 * acustici (beep) utilizzando la funzione tone() di Arduino.
 */

#include "buzzer.h"

/** * @brief Pin GPIO assegnato al buzzer.
 * @note La variabile è dichiarata @c static per limitarne lo scope a questo file.
 */
static uint8_t buzzerPin;

/**
 * @brief Inizializza il buzzer e configura il relativo pin.
 * * Configura il pin come @c OUTPUT. Se la macro @ref BUZZER_INIT_BEEP è impostata a 1,
 * esegue un beep di test all'avvio.
 ...
 * * Utilizza la generazione PWM per produrre un'onda quadra alla frequenza desiderata.
