 ...
 * * Questo header definisce la struttura @ref WeatherState, che funge da database
 * locale per le informazioni meteorologiche correnti e le previsioni a breve termine (7 giorni).
 */
 ...
 */
struct WeatherState {
    bool curValid  = false;     /**< True se i dati meteo correnti sono stati popolati correttamente. */
    ...
    uint8_t recvMask = 0;       /**< Maschera bitwise: il bit i-esimo è 1 se il giorno i è stato ricevuto. */

    uint32_t lastUpdateMs = 0;  /**< Timestamp (millis) dell'ultimo aggiornamento ricevuto. */
};

/** @brief Istanza esterna della struttura meteo, accessibile da altri moduli (es. UI). */
extern WeatherState wx;

/* --- Funzioni di Gestione --- */

/**
 * @brief Inizializza la struttura @ref WeatherState con valori nulli o non validi.
 */
void Weather_Init();
...
/**
 * @brief Traduce un codice numerico WMO in una stringa breve leggibile (es. "Rain").
